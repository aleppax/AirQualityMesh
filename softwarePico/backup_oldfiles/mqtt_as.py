...
class MQTT_base:
    ...
    def __init__(self, config_mqtt):
        self._events = config_mqtt["queue_len"] > 0
        # MQTT config_mqtt
        self._client_id = config_mqtt["client_id"]
        self._user = config_mqtt["user"]
        self._pswd = config_mqtt["password"]
        self._keepalive = config_mqtt["keepalive"]
        if self._keepalive >= 65536:
            raise ValueError("invalid keepalive time")
        self._response_time = config_mqtt["response_time"] * 1000  # Repub if no PUBACK received (ms).
        self._max_repubs = config_mqtt["max_repubs"]
        self._clean_init = config_mqtt["clean_init"]  # clean_session state on first connection
        self._clean = config_mqtt["clean"]  # clean_session state on reconnect
        will = config_mqtt["will"]
        if will is None:
            self._lw_topic = False
        else:
            self._set_last_will(*will)
        # WiFi config_mqtt
        self._ssid = config_mqtt["ssid"]  # Required for ESP32 / Pyboard D. Optional ESP8266
        self._wifi_pw = config_mqtt["wifi_pw"]
        self._ssl = config_mqtt["ssl"]
        self._ssl_params = config_mqtt["ssl_params"]
        # Callbacks and coros
        if self._events:
            self.up = asyncio.Event()
            self.down = asyncio.Event()
            self.queue = MsgQueue(config_mqtt["queue_len"])
        else:  # Callbacks
            self._cb = config_mqtt["subs_cb"]
            self._wifi_handler = config_mqtt["wifi_coro"]
            self._connect_handler = config_mqtt["connect_coro"]
        # Network
        self.port = config_mqtt["port"]
        if self.port == 0:
            self.port = 8883 if self._ssl else 1883
        self.server = config_mqtt["server"]
...
class MQTTClient(MQTT_base):
    def __init__(self, config_mqtt):
        ...
        p_i = config_mqtt["ping_interval"] * 1000  # Can specify shorter e.g. for subscribe-only
