...
def send_values():
    opms_logged,opensensemap_logged = filelogger.write(sensors.measures) # current measures immediately saved to file
    if cron.check_data_schedule(sensors.battery_values[2],sensors.leadacid.min_charging_voltage):
        ...
        if wlan.connect():
            ...
            while attempts > 0:
                attempts -= 1
                opms_file_lines, opensensemap_file_lines = filelogger.read()
